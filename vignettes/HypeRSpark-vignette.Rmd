...
The package relies on the HyperSpark framework. Consequently, its source code should be downloaded. The package function *dependencies()* downloads the most recent HyperSpark version from GitHub to the working directory. The source code is later used to package and execute the framework from R, after user configuration. Consequently, it is recommended that users use a dedicated project directory when developing HypeRSpark applications.
...
|:----------              |:-------------                     |:------------------|
| setProblem*             | PfsProblem, NrProblem, KpProblem  | An implemented problem, such as the PFSP. Should be the full name of the class.    |  
| data*                   | Data file                         | Data for the problem.   |   
| setStoppingCondition*   | TimeExpired                       | Maximum computation time per iteration    |   
...
| numOfAlgorithms         | Int                               | The number of parallel algorithms |
| setRandomSeed           | Int                               | Ensures reproducibility of results. |
| setInitialSeeds         | Array                             | Provide an array of solutions for nodes| 
| setNInitialSeeds        | Array (Solution) + int            | Provide a single solution, distributed to $N$ nodes|
| setNDefaultInitialSeeds | Int                               | Provide NONE seed to $N$ nodes | 
| setSeedingStrategy      | Problem specific                  | Define a seeding strategy | 
| setNumberOfIterations   | Int                               | Iterations to be performed in one run, default 1. Used for cooperative algorithms.|
| setMapReduceHandler     | MapReduceHandler, MapReduceHandlerMaximization  | MapReduce handler for minimization and maximization problems. | 
\* mandatory parameter

### Spark parameters
Besides the framework-specific parameters a number of Spark properties, displayed in the table below, can be defined. All Spark parameters are optional. Users will generally specify these argument when executing their application on a cluster, using [spark-submit](https://spark.apache.org/docs/latest/submitting-applications.html) However, it is good to note that the arguments can be configured in advance using the Spark parameters described below. 
...
| Parameter                        | Options    | Description                                          |
|:---------------------------------|:-----------|:-----------------------------------------------------|
| setSparkMaster                   | String     | Cluster URL                                          |  
...
| setDeploymentSpark               | -          | Cluster execution standalone resource manager        | 
| setDeploymentMesos               | -          | Cluster execution Mesos resource manager             |
| setDeploymentYarnClient          | -          | Cluster execution YARN resource manager client mode  |
| setDeploymentYarnCluster         | -          | Cluster execution YARN resource manager cluster mode |
...
The following configuration applies two algorithms on the PfsProblem: a Genetic Algorithm and Simulated Annealing. Furthermore, the maximum execution time in increased to 10 minutes.
...
This function packages the application and all its dependencies into a JAR archive, using Apache Maven. This JAR can be executed locally or on a cluster, depending on the configuration. Running the function for the first time can take a while, as all dependencies need to be downloaded from the central Maven repository. Subsequent packaging is much faster. The packaging process produces output which can be used to debug the configuration in case of errors. 




## 2.4 executeHyperSpark
HyperSpark can be executed locally, which is mainly used for testing, or on a cluster. The two following sections discuss both options. Note, however, that the package function *executeHyperSpark* can only be used for local execution, as cluster execution relies heavily on the provider. To execute the application locally, simply run the function:

```{r, eval=F}
hypeRSpark::executeHyperSpark()
```

The function output shows the progress, including any errors. Configuration errors can easily arise. The error usually hints towards where the error arises. 
